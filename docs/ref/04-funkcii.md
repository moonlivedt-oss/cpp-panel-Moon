...
**Как это устроено — что такое «тарелка» на самом деле.** Каждый вызов получает свой **кадр стека** (stack frame) — участок памяти, куда кладутся аргументы функции, её локальные переменные и **адрес возврата** (куда прыгнуть, когда функция закончится). Кадры лежат в особой области — **стеке** — и складываются буквально стопкой: новый вызов надстраивает свой кадр сверху, а процессор помнит вершину в отдельном регистре (указателе стека). Поэтому вход в функцию и выход почти бесплатны — надо лишь сдвинуть этот указатель, а не искать память по всей куче; и поэтому же локальные переменные живут ровно до конца функции — её кадр снимается, и память отдаётся под следующие вызовы. Стек не резиновый (обычно около 1 МБ): если кадров становится слишком много — например, при бесконечной рекурсии, — стопка упирается в потолок, и это и есть **переполнение стека**.
...
+ Через параметры-ссылки (или `struct`/`pair`)
- Написать два `return` подряд
- Через глобальные переменные
= Параметры-ссылки или пара/структура. Два `return` не выполнятся, глобальные переменные — плохая практика.
