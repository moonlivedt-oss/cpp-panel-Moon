...
> Определения функций этой темы (`getline`, `substr`, `find`, `stoi`) — коротко в [Словаре функций](10a-slovar-funkcij.md#тема-5-строки).
...
Если совсем с нуля: `std::string` — это **текст**: имя, слово, строка из файла. Его можно складывать через `+`, искать в нём, резать на части. Здесь — как это делать (и почему с русским всё сложнее, чем кажется).
...
<details>
<summary>Копнуть глубже: что такое строка внутри</summary>

`std::string` — это не сам текст, а маленький управляющий объект, который держит три вещи: **указатель** на буфер с байтами, **длину** (`size`) и **вместимость** (`capacity` — сколько байт уже выделено под рост). Сами символы лежат в непрерывном куске памяти; для длинных строк это участок в **куче** (heap), выделенный динамически. Поэтому `s.size()` мгновенный — длина хранится готовым числом, а не пересчитывается, — а `s += "…"` при нехватке места выделяет буфер побольше и копирует туда старое содержимое (ровно как `vector`, см. [как вектор растёт](06-konteynery.md#как-вектор-растёт-size-и-capacity)). Есть и оптимизация: очень короткие строки (обычно до 15 байт) многие реализации хранят прямо внутри самого объекта, без обращения к куче, — это «small string optimization», поэтому короткое имя или слово не стоят ни одного выделения памяти.
...
```quiz
В: Как узнать длину `std::string s`?
+ `s.size()` (или `s.length()`)
- `strlen(s)`
- `sizeof(s)`
= У строки есть `.size()`. `strlen` — для C-строк, `sizeof` вернёт размер объекта, а не длину текста.

В: Как взять подстроку с позиции 2 длиной 3?
+ `s.substr(2, 3)`
- `s.slice(2, 3)`
- `s[2..3]`
= `substr(pos, len)` — позиция и длина.

В: Как найти позицию символа `=` в строке?
+ `s.find('=')`
- `s.index('=')`
- `s.search('=')`
= `find` вернёт позицию или `std::string::npos`, если символа нет.

В: Чтобы изменить каждый символ строки НА МЕСТЕ, цикл пишут…
+ `for (char &c : s)` — по ссылке
- `for (char c : s)`
- `for (int c : s)`
= Только по ссылке `&`; иначе меняется копия символа, а строка остаётся прежней.

В: `char` в C++ — это…
+ целочисленный тип (код символа)
- отдельный «символьный» тип, не число
- строка из одного символа
= `char` — это маленькое целое, поэтому символы можно складывать и сравнивать.

В: Обход строки с конца по индексу. Каким должен быть тип индекса?
+ знаковый `int` (иначе `i >= 0` всегда истинно)
- `size_t`
- `unsigned`
= `s.size()` беззнаковый; для беззнакового `i` условие `i >= 0` истинно всегда → бесконечный цикл.
```
...
```findbug
std::string s = "hello";
for (int i = 0; i <= s.size(); ++i)   // печатаем по символу
    std::cout << s[i];
---
`i <= s.size()` обращается к `s[size()]` — за границей строки (допустимые индексы `0 .. size()-1`). Нужно строгое `i < s.size()`.
```

И «заполни пропуск» — впиши недостающее и нажми «Проверить»:

```fillcode
// поменять регистр каждого символа НА МЕСТЕ
for (char [[&]]c : s)             // без & менялась бы копия, а не строка
    c = static_cast<char>(std::toupper(static_cast<unsigned char>(c)));
```
...
```cards
Q: Как узнать длину строки?
A: `s.size()` (или `s.length()`).

Q: Почему у `"Привет"` `size()` == 12, а не 6?
A: в UTF-8 кириллица — 2 байта на букву, а `size()` считает байты. Русский текст по `s[i]` не режь.

Q: Как изменить каждый символ строки на месте?
A: перебор по ссылке: `for (char &c : s)`.

Q: Где `std::string` хранит длинный текст?
A: в куче (heap); сам объект держит указатель на буфер, длину (`size`) и вместимость (`capacity`).
```

> **Теперь потренируйся.** Теория освоена — закрепи её на задачах: [→ Задачник, тема 5. Строки](../zadachnik/05-stroki.md). Начни с 5.1 «Гласные в слове» и 5.2 «Переворот строки» (обе 🟢).
